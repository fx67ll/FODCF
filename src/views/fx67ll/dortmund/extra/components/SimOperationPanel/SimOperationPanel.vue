  ...
  <div class="status-card">
    ...
    <div class="sim-actions">
      ...
      <el-button type="info" size="small" icon="el-icon-refresh" plain
        @click="$emit('restart')">重新模拟</el-button>
    ...
    <el-alert v-if="simFinished"
      :title="`当前数值已归零，下一轮将从初始数值 ${formatSimNumber(simForm.initialValue)} 重新开始`" type="error" :closable="false"
      show-icon class="sim-rule-alert" />
    <el-alert v-else-if="simRestartPending"
      :title="`已重置，下一轮将从初始数值 ${formatSimNumber(simForm.initialValue)} 重新开始`" type="warning" :closable="false"
      show-icon class="sim-rule-alert" />
    <el-alert v-else-if="simTargetHit" title="已达成目标数值，可继续记录实际数值或重新模拟" type="success" :closable="false"
      show-icon class="sim-rule-alert" />
  </div>
</template>
